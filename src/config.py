"""
Single source of truth for anything that must be IDENTICAL between
Part A (traditional CNN) and Part B (custom CNN), so the comparison
stays fair. If you change EPOCHS for Part B, you must also update
JUSTIFY_EPOCH_CHANGE below and mention it in the report — the code
will refuse to silently drift.
"""
...
# --- Data split (reused identically in Part A and Part B) ------------------
...
# --- Training budget ---------------------------------------------------------
BATCH_SIZE = 64
EPOCHS = 20  # shared training budget for Part A and Part B

# If Part B needs more epochs, set this explicitly (fails loudly otherwise).
JUSTIFY_EPOCH_CHANGE = None  # e.g. "LR schedule needs 5 extra epochs to converge"
CUSTOM_EPOCHS = EPOCHS  # override only together with JUSTIFY_EPOCH_CHANGE
